...
const AdminValidation: React.FC<AdminValidationProps> = ({ store }) => {
  const pendingRequests = store.entries.filter(e => 
    e.type === EntryType.EXPENSE_MAINTENANCE && e.status === MaintenanceStatus.PENDING
  );
              ...
              <div key={request.id} className="bg-neutral-900/50 border border-neutral-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col lg:flex-row transition-all hover:border-neutral-700">
                ...
                <div className="flex-1 p-8 flex flex-col">
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
                    <div>
                      <h3 className="font-black text-2xl text-neutral-100 uppercase tracking-tighter">{request.maintenanceType}</h3>
                      ...
                      <div className="flex items-center gap-2 mt-4">
                         ...
                         <p className="text-[9px] text-neutral-500 font-bold uppercase">Saisi par <span className="text-neutral-300">{request.userName}</span> • {new Date(request.date).toLocaleString('fr-FR')}</p>
                      </div>
                    </div>
                    <div className="text-right w-full sm:w-auto p-4 bg-neutral-950 rounded-2xl border border-neutral-800">
                      ...
                      <p className="text-2xl font-black text-red-500">{request.amount.toLocaleString()} <span className="text-xs text-neutral-600">{CURRENCY}</span></p>
                      <p className="text-[10px] text-neutral-400 font-black uppercase mt-1">@ {request.mileageAtEntry?.toLocaleString()} KM</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-neutral-950/50 p-6 rounded-2xl mb-8 border border-neutral-800">
                    ...
                    <p className="text-sm text-neutral-300 italic">"{request.info || 'Aucune observation enregistrée pour cette opération.'}"</p>
                  </div>
